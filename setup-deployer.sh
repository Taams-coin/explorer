...
## Config
rm -rf "$HOME/.deployer/taams/"
mkdir -p "$HOME/.deployer/taams/"
CONFIG_PATH="$HOME/.deployer/taams/config.json"
cat > "$CONFIG_PATH" <<- EOF
{
  "coreIp": "127.0.0.1",
  "p2pPort": 4002,
  ...
  "gitCoreCommit": true,
  "gitCoreOrigin": "https://github.com/Taams-coin/wallet",
...
if [ "$NETWORK" == "mainnet" ]; then
    cp "$HOME/.bridgechain/mainnet/taams/delegates.json" "$HOME/.config/taams-core/mainnet/"
elif [ "$NETWORK" == "devnet" ]; then
    cp "$HOME/.bridgechain/devnet/taams/delegates.json" "$HOME/.config/taams-core/devnet/"
fi
